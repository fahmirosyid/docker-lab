...
def index():
    ...
    return jsonify({"service": "flask-app", "status": "running", "hostname": socket.gethostname()})
...
def log_stats():
    try:
        ...
        cur.execute("""
            SELECT tag, COUNT(*) AS count FROM logs.fluentbit
            WHERE time > NOW() - INTERVAL '1 hour'
            GROUP BY tag ORDER BY count DESC
        """)
        by_tag = [{"tag": r[0], "count": r[1]} for r in cur.fetchall()]
        cur.execute("""
            SELECT (data->>'log')::jsonb->>'level' AS level, COUNT(*) AS count
            FROM logs.fluentbit
            WHERE time > NOW() - INTERVAL '1 hour'
        ...
        return jsonify({"total_logs_all_time": total, "last_hour_by_tag": by_tag, "last_hour_by_level": by_level})
...
def log_search():
    ...
    try:
        conn = get_db(); cur = conn.cursor()
        query = """
            SELECT id, received_at, tag, container_name, log_level, message, service
            FROM logs.structured_logs WHERE 1=1
        """
        params = []
        if keyword:
        ...
        logs = [{"id": r[0], "time": str(r[1]), "tag": r[2], "container": r[3], "level": r[4], "message": r[5], "service": r[6]} for r in cur.fetchall()]
        cur.close(); conn.close()
...
def log_raw():
    ...
    try:
        ...
        cur.execute("""
            ...
            FROM logs.fluentbit ORDER BY time DESC LIMIT %s
        """, (limit,))
        logs = [{"id": r[0], "time": str(r[1]), "tag": r[2], "container": r[3], "source": r[4], "log": r[5]} for r in cur.fetchall()]
        cur.close(); conn.close()
        return jsonify({"count": len(logs), "results": logs})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
